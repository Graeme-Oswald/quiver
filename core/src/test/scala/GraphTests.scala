...
object GraphTests extends Properties("Graph") {
  ...
  locally {
    val _ = property("embed and decomp are inverses of each other") =
  ...
  locally {
    val _ = property("Adding a node to a graph yields a graph that contains that node") =
  ...
  locally {
    val _ = property("Adding a node and removing it again yields a graph that doesn't contain that node") =
      forAll { (g: Graph[N, Int, Int], n: LNode[N, Int]) =>
  ...
  locally {
    val _ = property("Adding an edge should yield a map that contains that edge") =
  ...
  locally {
    val _ = property("Adding an edge and removing it again yields a graph that doesn't contain that edge") =
      forAll { (g: Graph[N, Int, Int], e: LEdge[N, Int]) =>
  ...
  locally {
    val _ =   property("A graph constructed from nodes and edges should contain those nodes and edges") =
  ...
  locally {
    val _ = property("A graph constructed from another graph's nodes and edges should be the same graph") =
  ...
  locally {
    val _ = property("Adding an edge between two existing nodes should succeed") =
  ...
  locally {
    val _ = property("DFS should find all descendants of a node") =
  ...
  locally {
    val _ = property("RDFS should find all ancestors of a node") =
      forAll { (ns: List[LNode[N, Int]], l: Int) =>
        val g = connect(ns, l)
  ...
  locally {
    val _ = property("BFS should find all descendants of a node") =
      forAll { (ns: List[LNode[N, Int]], l: Int) =>
        val g = connect(ns, l)
  ...
  locally {
    val _ = property("The union of two graphs should contain all the edges and nodes of both graphs") =
      forAll { (g1: Graph[N, Int, Int], g2: Graph[N, Int, Int]) =>
        val u = g1 union g2
        g1.edges.forall(u.edges contains _) &&
          g1.nodes.forall(u.nodes contains _) &&
          g2.edges.forall(u.edges contains _) &&
          g2.nodes.forall(u.nodes contains _)
      }
  }
  ...
  locally {
    val _ = property("Union is commutative (ignoring labels)") =
      forAll { (g1: Graph[N, Int, Int], g2: Graph[N, Int, Int]) =>
        val u1 = g1 union g2
        val u2 = g2 union g1
  ...
  locally {
    val _ = property("Folding on contexts equals folding on nodes and edges") =
  ...
  locally {
    val _ = property("The roots of a graph have no incoming edges from other nodes") = forAll {
      (ns: List[LNode[N, Int]], es: List[LEdge[N, Int]], _: Int) =>
        val g = safeMkGraph(ns, es)
  ...
  locally {
    val _ = property("The leaves of a graph have no outgoing edges to other nodes") = forAll {
      (ns: List[LNode[N, Int]], es: List[LEdge[N, Int]], _: Int) =>
        val g = safeMkGraph(ns, es)
        val vs = ns.map(_.vertex).toSet
  ...
  locally {
    val _ = property("The shortest path should be the cheapest path with constant cost") = forAll {
  ...
  locally {
    val _ = property("The shortest path through a graph should be the shortest path between all subpaths") = forAll {
      (tpg: (Graph[N, Int, Int], LNode[N, Int], LNode[N, Int])) =>
        val start = tpg._2
        val end = tpg._3
        val graph = tpg._1
  ...
  locally {
    val _ = property("The shortest labelled path through a graph should be labelled by the labelled shortest labelled path between all subpaths") = forAll {
      (tpg: (Graph[N, Int, Int], LNode[N, Int], LNode[N, Int])) =>
        val start = tpg._2
        val end = tpg._3
        val graph = tpg._1
  ...
  locally {
    val _ = property("The shortest labelled path through a graph without the labels should be the shortest path exactly") = forAll {
      (tpg: (Graph[N, Int, Int], LNode[N, Int], LNode[N, Int])) =>
        val start = tpg._2
        val end = tpg._3
        val graph = tpg._1
        ...
        //s"Labelled path is $lpath which should be $path if the labels are dropped" |:
        lpath.map { case (s, p) => s +: p.map(_._1) } === path
    }
  }
}
